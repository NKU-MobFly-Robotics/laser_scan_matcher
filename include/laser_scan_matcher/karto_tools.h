/**
 * karto_tools include some useful modules which I decoupled from open_karto library
 * 
 * \a Size2 \n
 ...
 *    (x, y) is at bottom left in rectangle
 * 
 * \a Pose2 \n
 ...
 * 
 *    The user can set an offset pose for the sensor relative to the robot coordinate system. If no value 
 *    is provided by the user, the sensor is set to be at the origin of the robot coordinate system.
 * 
...
#ifndef _KARTO_TOOLS_H_
#define _KARTO_TOOLS_H_
 ...
 * 
 * The user can set an offset pose for the sensor relative to the robot coordinate system. If no value is provided by the user, the sensor is set to be at the origin of the robot coordinate system.
 * 
 * The LaserRangeFinder contains parameters for physical laser sensor used by the mapper for scan matching.
 * Also contains information about the maximum range of the sensor and provides a threshold for limiting the range of readings.
 * 
 * The optimal value for the range threshold depends on the angular resolution of the scan and the desired map resolution.  RangeThreshold should be set as large as possible while still providing "solid" coverage between consecutive range readings.
 */
...
private:
	...
	void Update()
	{
		m_NumberOfRangeReadings = static_cast<int>(math::Round((GetMaximumAngle() -
																	GetMinimumAngle()) / 
