...
class Size2 {
 ...
 public:
   ...
   * Equality operator
   */
  bool operator==(const Size2& rOther) const {
...
class Vector2 {
 ...
 public:
   ...
   */
  double SquaredDistance(const Vector2& rOther) const {
   ...
   */
  double Distance(const Vector2& rOther) const {
 ...
 public:
   ...
   * @return dot product
   */
  double operator*(const Vector2& rOther) const {
   ...
   * @param rOther
   */
  bool operator==(const Vector2& rOther) const {
...
class Rectangle2 {
 ...
 public:
   ...
   */
  void SetPosition(const Vector2<T>& rPosition) { m_Position = rPosition; }
   ...
   * @param rSize size
   */
  void SetSize(const Size2<T>& rSize) { m_Size = rSize; }
 ...
 public:
   ...
   * Equality operator
   */
  bool operator==(const Rectangle2& rOther) const {
...
class Pose2 {
 ...
 public:
   ...
   */
  const Vector2<double>& GetPosition() const { return m_Position; }

  /**
   * Sets the position
   * @param rPosition of the pose
   */
  void SetPosition(const Vector2<double>& rPosition) { m_Position = rPosition; }
   ...
   */
  double SquaredDistance(const Pose2& rOther) const {
 ...
 public:
   ...
   * Equality operator
   */
  bool operator==(const Pose2& rOther) const {
...
class Matrix3 {
 ...
 public:
   ...
   */
  double& operator()(int row, int column) { return m_Matrix[row][column]; }
   ...
   * @return mat(r,c)
   */
  double operator()(int row, int column) const { return m_Matrix[row][column]; }
...
class BoundingBox2 {
 ...
 public:
   ...
   */
  const Vector2<double>& GetMinimum() const { return m_Minimum; }

  /**
   * Set bounding box minimum
   */
  void SetMinimum(const Vector2<double>& mMinimum) { m_Minimum = mMinimum; }

  /**
   * Get bounding box maximum
   */
  const Vector2<double>& GetMaximum() const { return m_Maximum; }

  /**
   * Set bounding box maximum
   */
  void SetMaximum(const Vector2<double>& rMaximum) { m_Maximum = rMaximum; }

  /**
   * Get the size of the bounding box
   */
  Size2<double> GetSize() const {
   ...
   */
  void Add(const Vector2<double>& rPoint) {
   ...
   * @return in bounds?
   */
  bool IsInBounds(const Vector2<double>& rPoint) const {
...
class Transform {
 public:
   ...
   * @param rPose pose
   */
  explicit Transform(const Pose2& rPose) { SetTransform(Pose2(), rPose); }
...
class CoordinateConverter {
 ...
 public:
   ...
   * @return scaled value
   */
  double Transform(double value) { return value * m_Scale; }
   ...
   */
  Vector2<int> WorldToGrid(const Vector2<double>& rWorld,
   ...
   */
  Vector2<double> GridToWorld(const Vector2<int>& rGrid,
                              bool flipY = false) const {
   ...
   * @return offset
   */
  const Vector2<double>& GetOffset() const { return m_Offset; }

  /**
   * Sets the offset
   * @param rOffset
   */
  void SetOffset(const Vector2<double>& rOffset) { m_Offset = rOffset; }

  /**
   * Sets the size
   * @param rSize
   */
  void SetSize(const Size2<int>& rSize) { m_Size = rSize; }

  /**
   * Gets the size
   * @return size
   */
  const Size2<int>& GetSize() const { return m_Size; }

  /**
   * Gets the resolution
   * @return resolution
   */
  double GetResolution() const { return 1.0 / m_Scale; }

  /**
   * Sets the resolution
   * @param resolution
   */
  void SetResolution(double resolution) { m_Scale = 1.0 / resolution; }
...
class LaserRangeScan {
 public:
   ...
   * @param rSensorName
   */
  LaserRangeScan() : m_pRangeReadings(nullptr), m_NumberOfRangeReadings(0) {}
   ...
   */
  explicit LaserRangeScan(const RangeReadingsVector& rRangeReadings)
      : m_pRangeReadings(nullptr), m_NumberOfRangeReadings(0) {
 ...
 public:
   ...
   */
  double* GetRangeReadings() const { return m_pRangeReadings; }
   ...
   */
  int GetNumberOfRangeReadings() const { return m_NumberOfRangeReadings; }
...
class LaserRangeFinder {
 ...
 public:
   ...
   * @return offset pose
   */
  const Pose2& GetOffsetPose() const { return m_OffsetPose; }

  /**
   * Sets this range finder sensor's offset
   * @param rPose
   */
  void SetOffsetPose(const Pose2& rPose) { m_OffsetPose = rPose; }

  /**
   * Gets this range finder sensor's minimum range
   * @return minimum range
   */
  double GetMinimumRange() const { return m_MinimumRange; }
   ...
   */
  double GetMaximumRange() const { return m_MaximumRange; }
   ...
   */
  double GetRangeThreshold() const { return m_RangeThreshold; }
   ...
   */
  double GetMinimumAngle() const { return m_MinimumAngle; }
   ...
   */
  double GetMaximumAngle() const { return m_MaximumAngle; }
...
class LocalizedRangeScan : public LaserRangeScan {
 ...
 public:
   ...
   */
  const Pose2& GetOdometricPose() const { return m_OdometricPose; }

  /**
   * Sets the odometric pose of this scan
   * @param rPose
   */
  void SetOdometricPose(const Pose2& rPose) { m_OdometricPose = rPose; }
   ...
   */
  const Pose2& GetCorrectedPose() const { return m_CorrectedPose; }
   ...
   * @return scan pose
   */
  Pose2 GetSensorPose() const { return GetSensorAt(m_CorrectedPose); }
   ...
   */
  const PointVectorDouble& GetPointReadings(bool wantFiltered = false) const {
    boost::shared_lock<boost::shared_mutex> lock(m_Lock);
    if (m_IsDirty) {
      // throw away constness and do an update!
      lock.unlock();
   ...
   */
  LaserRangeFinder* GetLaserRangeFinder() const { return m_pLaserRangeFinder; }
...
class Grid {
 ...
 public:
   ...
   * @param rGrid
   */
  bool IsValidGridIndex(const Vector2<int>& rGrid) const {
   ...
   */
  const Size2<int> GetSize() const { return Size2<int>(m_Width, m_Height); }

  /**
   * Gets the width step in bytes
   * @return width step
   */
  int GetWidthStep() const { return m_WidthStep; }
   ...
   * @return data pointer
   */
  T* GetDataPointer() const { return m_pData; }

  /**
   * Gets the allocated grid size in bytes
   * @return data size
   */
  int GetDataSize() const { return m_WidthStep * m_Height; }
   ...
   * @return value
   */
  T GetValue(const Vector2<int>& rGrid) const {
 ...
 protected:
  ...
  Grid(int width, int height)
      : m_pData(nullptr), m_pCoordinateConverter(nullptr) {
    Resize(width, height);
  }
...
class ScanManager {
 ...
 public:
   ...
   */
  LocalizedRangeScan* GetLastScan() { return m_pLastScan; }

  /**
   * Sets the last scan
   * @param pScan
   */
  void SetLastScan(LocalizedRangeScan* pScan) { m_pLastScan = pScan; }

  /**
   * Gets scans
   * @return scans
   */
  LocalizedRangeScanVector& GetScans() { return m_Scans; }

  /**
   * Gets running scans
   * @return running scans
   */
  LocalizedRangeScanVector& GetRunningScans() { return m_RunningScans; }
